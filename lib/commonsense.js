...
(function() {
  ...
  Sense.prototype = {
    ...
    findOrCreateSensor: function(sensor, next) {
      var findSensor, per_page, self;
      self = this;
      per_page = 1000;
      findSensor = function(page, cb) {
        return self.sensors({
          page: page,
          per_page: per_page
        }, function(error, response) {
          var attr, c, data, found, same, sensors, _i, _j, _len, _len1, _ref;
          if (response.status === 200) {
            ...
            for (_i = 0, _len = sensors.length; _i < _len; _i++) {
              c = sensors[_i];
              same = true;
              _ref = ['name', 'display_name', 'device_type', 'pager_type', 'data_type', 'data_structure'];
          ...
          if (response.status === 409 || response.status === 509) {
            return setTimeout(function() {
              return findSensor(page, cb);
            }, 60000);
          }
        });
      };
      return findSensor(0, function(sensorResult) {
        if (sensorResult === null) {
          return self.createSensor({
            "sensor": sensor
          }, function(error, response) {
            if (response.status === 201) {
              return next(response.object.sensor);
            }
          });
        } else {
          return next(sensorResult);
        }
      });
    },
